<?php
  ...
  $height = (osm_param('height_search') <= 0 ? 360 : osm_param('height_search'));
...
<?php if(osc_count_items() > 0 && !empty($lats)) { ?>
  ...
  <?php if(osm_param('fullscreen_search') == 1) { ?>
  ...
  <script>
    var osmMap = L.map('searchMap'<?php if(osm_param('fullscreen_search') == 1) { ?>, {fullscreenControl:{pseudoFullscreen:true}}<?php } ?>).setView([40.6971,-74.2598], <?php echo $zoom; ?>);
